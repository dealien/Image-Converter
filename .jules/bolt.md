...
## 2024-05-25 - Safely Caching Mutable Objects (Lists) with lru_cache
**Learning:** While returning mutable objects (like `list`) from an `@functools.lru_cache` decorated function is generally dangerous because caller mutations will corrupt the cache, it is safe when the returned list is exclusively passed to read-only C-level functions like PIL's `image.point()`. By extracting list concatenation (e.g. `lut = lut_channel * 3 + _IDENTITY_LUT`) into a cached helper function based on image `mode` and `brightness` values, we completely avoid the repeated memory allocation and concatenation overhead that was costing ~410µs per RGB image.
**Action:** When assembling multi-channel LUTs for dynamic transformations, extract the full list concatenation into a separate helper function decorated with `@functools.lru_cache`, keyed on the mode and adjustment parameters, to prevent redundant list allocations on every call.
